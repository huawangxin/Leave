...
<form id="form2" action="<%=request.getContextPath() %>/leave/basicoffduty/${basicOffduty.id }/update" method="post">
    <ul class="forminfo">
    <li>
    <input type="hidden" name="id" value="${basicOffduty.id }"/>
    <input type="hidden" name="overtime" value="${basicOffduty.overtime }"/>
    <label>姓名</label>
     <label class="dfinput" style="width:345px;height:30px">${basicOffduty.name }</label><br/></li>
    <li><label>部门</label>
    <select name="department"  class="dfinput">
           <option value="${basicOffduty.department }">${basicOffduty.department }</option>
    ...
    <li><label>调休时间</label>
    <input name="ovetimeStart"  id="dt"  class="easyui-datetimebox"  editable="false"  value="${basicOffduty.ovetimeStart }"
    data-options="formatter:ww4,parser:w4" style="width:200px;"/>
   到<input name="ovetimeOver"  id="dt1"   class="easyui-datetimebox"  editable="false"  value="${basicOffduty.ovetimeOver }"
   data-options="formatter:ww4,parser:w4" style="width:200px;"/>
<!--     一共<input name="leaveTime" value="" type="text"/>天 -->
	<i class="tips" id="divoverTime"></i>
    </li>
    <li><label>调休事由</label><textarea name="reason" id="reason" cols="" rows="" class="textinput"></textarea>
...
reason.innerHTML="${basicOffduty.reason }";
</script>

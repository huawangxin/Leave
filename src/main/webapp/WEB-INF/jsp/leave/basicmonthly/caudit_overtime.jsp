...
<!--     <li><a id="t1" href="#tab1">请假月报</a></li> -->
    <li><a id="t2"  href="#tab2" class="selected">加班月报</a></li>
<!--     <li><a id="t3"  href="#tab3">调休月报</a></li> -->
    ...
    <form id="form2" method="post" action="<%=request.getContextPath() %>/leave/basicovertime/search">
    <ul class="seachform">
    ...
    <table class="tablelist">
        ...
        <tr>
        <td>${basicOvertime.department}</td>
        <td>${basicOvertime.name}</td>
        <td>${basicOvertime.ovetimeStart} 至  ${basicOvertime.ovetimeOver} 共计 ${basicOvertime.overtime} 天</td>
        <td>${basicOvertime.reason}</td>
        <td>${basicOvertime.place}</td>
        <td><a href="<%=request.getContextPath() %>/leave/basicovertime/${basicOvertime.id}" class="tablelink">查看</a></td>
        </tr>
        </c:forEach>
   ...
   <%  if(pages.getCount()==0){
    ...
    <div class="pagin">
        ...
        <ul class="paginList">
        <li><a href="<%=request.getContextPath() %>/leave/basicovertime/listbefore?curPage=1&pageCount=${page_basicOvertime.pageCount}">首页</a>	<a href="<%=request.getContextPath() %>/leave/basicovertime/listbefore?curPage=${page_basicOvertime.curPage}&pageCount=${page_basicOvertime.pageCount}">上一页</a>	第 ${page_basicOvertime.curPage }/${page_basicOvertime.pageCount==0?1:page_basicOvertime.pageCount } 页	<a href="<%=request.getContextPath() %>/leave/basicovertime/listnext?curPage=${page_basicOvertime.curPage}&pageCount=${page_basicOvertime.pageCount}">下一页</a>	<a href="<%=request.getContextPath() %>/leave/basicovertime/listnext?curPage=${page_basicOvertime.pageCount}&pageCount=${page_basicOvertime.pageCount}">尾页</a></li>
        </ul>
    </div>
   		<%
   	} %>

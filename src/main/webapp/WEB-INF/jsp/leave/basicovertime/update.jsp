...
<form id="form2" action="<%=request.getContextPath() %>/leave/basicovertime/${basicOvertime.id}/update" method="post">
    <ul class="forminfo">
    <li><label>姓名</label>
    <input type="hidden" name="id" value="${basicOvertime.id }"/>
    ...
    <select name="department"  class="dfinput">
           <option value="${basicUser.department }">${basicOvertime.department }</option>
    ...
    <li><label>加班地点</label><input name="place" type="text" value="${basicOvertime.place }"
     onKeypress="javascript:if(event.keyCode == 32)event.returnValue = false;" class="dfinput" />
     <i class="tips" id="divplace"></i>
     </li>
    <li><label>加班时间</label>
    <input name="ovetimeStart"  id="dt"  class="easyui-datetimebox"  editable="false"  value="${basicOvertime.ovetimeStart }"
    data-options="formatter:ww4,parser:w4" style="width:200px;"/>
   到<input name="ovetimeOver"  id="dt1"   class="easyui-datetimebox"  editable="false"   value="${basicOvertime.ovetimeOver }"
   data-options="formatter:ww4,parser:w4" style="width:200px;"/>
<!--     一共<input name="leaveTime" value="" type="text"/>天 -->
	<i class="tips" id="divoverTime"></i>
    </li>
    <li><label>加班事由</label><textarea name="reason" id="reason" cols="" rows="" class="textinput"></textarea>
...
reason.innerHTML="${basicOvertime.reason }";
</script>
